...
```bash
kubectl rollout status deployment/web   # blocks indefinitely
kubectl get pods                          # new pod shows ImagePullBackOff
```
...
```bash
kubectl exec <pod> -- ls /etc/config      # shows: db_host  db_port
kubectl exec <pod> -- cat /etc/config/db_host  # shows: postgres
```
...
```bash
kubectl exec <pod> -- ls -la /etc/secret   # permissions: -r--------
kubectl exec <pod> -- cat /etc/secret/username  # decoded plain text
```
...
# Shows: data-redis-0, data-redis-1, data-redis-2
```
...
containers:
  - name: main
    image: busybox
    command: ["sh","-c","cat /data/ready; sleep 3600"]
    volumeMounts:
      - name: shared
        mountPath: /data
volumes:
...
```bash
kubectl exec <pod> -- touch /mnt/data/test
# Output: touch: /mnt/data/test: Read-only file system
```
...
```bash
kubectl run debug --rm -it --image=busybox -- nslookup redis-headless
...
# Short name fails (resolves in current namespace 'staging'):
nslookup web
# Returns: server can't find web
...
kubectl label pods -l app=app-a tier=frontend
```

# Deployment B

```bash
kubectl create deployment app-b --image=nginx:1.27
kubectl label pods -l app=app-b tier=frontend
```

# Service selecting on the shared label

```bash
kubectl create service clusterip frontend --tcp=80:80
kubectl patch service frontend -p '{"spec":{"selector":{"tier":"frontend"}}}'
```
...
# Look for: Liveness: http-get http://:80/ delay=10s timeout=1s period=5s
```
...
```bash
kubectl describe pod <pod-name>
# Look for: Readiness: tcp-socket :6379 delay=5s timeout=1s period=10s
```
...
```bash
kubectl describe pod <pod-name>
# Events will show: Failed to pull image ... 404 Not Found, etc.
```
...
# Shows: OOMKilled in the Last State section
```
...
template:
  metadata:
    labels:
      app: web        # must contain all selector keys
```
...
```bash
kubectl describe pod <pod>
# Events: persistentvolumeclaim not bound
```

```bash
kubectl get pvc <pvc-name>   # NotFound
```
...
```bash
kubectl describe pod <pod>
# Events: Error: couldn't find key db_host in ConfigMap default/app-config
```
...
# Events: ReplicaSet never available within progress deadline
```
